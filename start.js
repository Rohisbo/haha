...
zeroyt7.sendMessage(anu.id, { image: { url: buff }, contextInfo: { mentionedJid: [num] }, caption: `Welcome To *${metadata.subject}* @${num.split("@")[0]}
...
let buffer = await getBuffer(`https://hardianto.xyz/api/goodbye4?profile=${ppuser}&name=${num.split("@")[0]}`)
zeroyt7.sendMessage(anu.id, { image: { url: buffer }, contextInfo: { mentionedJid: [num] }, caption: `Yah Kok Keluar Dari *${metadata.subject}* Mental aman kan kak, Sayonara @${num.split("@")[0]} :(` })
}
}
} catch (err) {
console.log(err)
